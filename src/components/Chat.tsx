...
export function Chat({ session, privateKey, initialContact, isPartnerOnline, onBack, onInitiateCall, isFriend = true, onSendFriendRequest }: ChatProps) {
  ...
  const updateAutoDeleteMode = async (mode: string) => {
    ...
    // Update for both users to keep it fixed for both partners
    const updates = [
  ...
  const decryptMessageContent = async (msg: any) => {
    try {
      ...
      try {
        ...
        // Fallback for legacy clear-text messages if any
      ...
      if (!error && data) {
        ...
        setMessages(decryptedMessages);
          ...
          if (unviewed.length > 0) {
            ...
            // Batch update using upsert or multiple calls. Supabase update with multiple IDs is better:
  ...
  const sendMessage = async (mediaType: string = "text", mediaUrl: string | null = null, mediaIv: string | null = null) => {
    ...
    try {
      ...
      if (!error && data?.[0]) {
        const sentMsg = { ...data[0], decrypted_content: contentToEncrypt };
